...
import { Search, Eye, Building, User, FileText, Gavel, Clock, CheckCircle, XCircle, AlertTriangle } from "lucide-react"
...
const mockBusinessUsers = [
  {
    ...
    documents: ["trade-license.pdf", "tax-certificate.pdf", "bank-statement.pdf"],
    role: "vendor",
  },
  {
    id: "BUS-002",
  ...
  },
  {
    ...
    documents: ["trade-license.pdf", "medical-certifications.pdf", "iso-certificate.pdf"],
    role: "vendor",
  },
  {
    id: "BUS-004",
...
export function UsersContent() {
  ...
  const currentUsers = activeTab === "businesses" ? mockBusinessUsers : mockIndividualUsers
  ...
  const handleUpgradeRole = (userId) => {
    console.log("Upgrading user role:", userId)
  }

  const getTotalUsers = (type) => {
    return type === "businesses" ? mockBusinessUsers.length : mockIndividualUsers.length
  }

  const getActiveUsers = (type) => {
    const users = type === "businesses" ? mockBusinessUsers : mockIndividualUsers
    return users.filter((u) => u.status === "active").length
  }

  const getPendingKyc = (type) => {
    const users = type === "businesses" ? mockBusinessUsers : mockIndividualUsers
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {[...mockBusinessUsers, ...mockIndividualUsers].filter((u) => u.status === "active").length}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-yellow-600">
              {[...mockBusinessUsers, ...mockIndividualUsers].filter((u) => u.kycStatus === "pending").length}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-gray-900">{mockBusinessUsers.length}</div>
